...
def check_programs_vs_grilla_tn(playlist, grilla_pairs, current_start, lang):
    ...
    # Episode counts from grilla (preserves re-airs)
    grilla_eps   = {}   # ep_num -> show_name (first occurrence, for display)
    grilla_count = Counter()  # ep_num -> count of scheduled airings
    for show_name, ep_num in grilla_pairs:
        grilla_count[ep_num] += 1
        if ep_num not in grilla_eps:
            grilla_eps[ep_num] = show_name

    # Count airings from JSON using name-change tracking
    # Each time p['name'] changes = new show block (handles re-airs)
    json_eps    = {}   # ep_num -> first occurrence
    ...
    for p in playlist['programs']:
        ...
        if ep_num is None: prev_name = name; continue
        if name != prev_name:
            json_counter[ep_num] += 1
            if ep_num not in json_eps:
                json_eps[ep_num] = {'name': name, 'start': p['start']}
        prev_name = name

    issues = []
    # Compare grilla counts vs json counts
    all_eps = sorted(set(grilla_count) | set(json_counter))
    for ep in all_eps:
        gc   = grilla_count.get(ep, 0)
        jc   = json_counter.get(ep, 0)
        show = grilla_eps.get(ep, f'ep{ep}')
        info = json_eps.get(ep, {})
        if gc == jc: continue
        if gc > jc:
            diff = gc - jc
            issues.append(f'  ✗  NOT IN PLAYLIST: {show} ep{ep}'
                          + (f' ({diff}x missing)' if diff > 1 else ''))
        else:
            diff = jc - gc
            t_str = fmt_t(info['start']) if info.get('start') else '?'
            issues.append(f'  ✗  EXTRA IN PLAYLIST: {info.get("name", f"ep{ep}")} @ {t_str} (not in grilla)'
                          + (f' ({diff}x extra)' if diff > 1 else ''))
    if not issues:
        issues.append(f'  ✓  All {len(grilla_count)} episodes match grilla')
    return issues
